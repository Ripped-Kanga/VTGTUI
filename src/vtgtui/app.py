...
class FileBrowserScreen(ModalScreen):
    """Textual-native file browser for selecting video files.

    Used on Linux terminals where native dialog tools (zenity, kdialog,
    tkinter) may not be available or broken.
    """
    ...
    def compose(self) -> ComposeResult:
        with VerticalScroll(id="fb-dialog"):
            yield Label("Select a Video File", id="fb-title")
            yield _VideoTree(Path.home(), id="fb-tree")
    ...
    def on_directory_tree_file_selected(
        ...
        """Fired when the user presses Enter on a file node — auto-confirm."""
        if is_supported_format(event.path):
    ...
    def on_tree_node_highlighted(self, event: Tree.NodeHighlighted) -> None:
        """Update status bar and Select button as the user moves through the tree."""
        try:
...
class VTGApp(App):
    ...
    def action_browse(self) -> None:
        ...
        if sys.platform in ("darwin", "win32"):
            # Use native OS dialog for macOS / Windows
            browse_btn = self.query_one("#browse-btn", Button)
            browse_btn.disabled = True
            self._do_browse()
        else:
            # Use the built-in Textual file browser for Linux/other terminals
    ...
    def _on_file_browser_result(self, path: str | None) -> None:
        """Handle result from the Textual-native file browser."""
        self.query_one("#frame-preview", FramePreview).restore_kitty()
        if path:
            self.set_input_file(path)
        else:
    ...
    def _do_browse(self) -> None:
        """Run a native OS file-picker dialog in a background thread (macOS/Windows)."""
